...
from function.function import ContrastiveLoss, seed_func, cal_accuracy_fewshot_ensemble_5shot
...
if args.dataset == 'CWRU':
    ...
    acc = cal_accuracy_fewshot_ensemble_5shot(test_dataloader_CWRU, net, device = args.device)
...
if args.dataset == 'PDB':
    ...
    net = MainNet().to(args.device)
    net = torch.load(args.best_weight)
    acc = cal_accuracy_fewshot_ensemble_5shot(test_dataloader_PDB, net, device = args.device)
    print('Accuracy on CWRU test data:', acc)
